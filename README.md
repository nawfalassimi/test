...
Layered — each layer only talks to its neighbors through a clean interface.
Config + registry driven — strategies, signals, and risk limits are defined
in YAML/config and dispatched to registered classes, not hardcoded branches.
One immutable "market snapshot" per date flows through the pipeline so every
...
Contenu du projetBacktestCréé par vousAjoutez des PDF, des documents ou autres textes à référencer dans ce projet.
